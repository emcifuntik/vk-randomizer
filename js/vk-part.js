var debug = false;
var contest = {type: 'repost', winners: new Array(), winnersCount: 0, needWinners: 0, owner: '', post: '', repostsCount: 0, checkSubscribtion: false};
...
function getWinners() {
	...
	if (checkInput()) {
		...
		$.post('count.php', {user: userID, post: contest.owner + "_" + contest.post});
...
function getLikesCount() {
	VK.api('likes.getList',{type: 'post', owner_id: contest.owner, item_id: contest.post, filter: 'likes', friends_only: '0', extended: '0', offset: '0', count: '1', skip_own: '1'},setRepostsCount);
	}

function getRepostsCount() {
	VK.api('likes.getList',{type: 'post', owner_id: contest.owner, item_id: contest.post, filter: 'copies', friends_only: '0', extended: '0', offset: '0', count: '1', skip_own: '1'},setRepostsCount);
	}
...
function getRepost() {
	...
	VK.api('wall.getReposts',{type: 'post', owner_id: contest.owner, post_id: contest.post, offset: getRandom(), count: '1'}, workRepost);
	}

function workRepost(data) {
	if (typeof data.response != "undefined" && data.response.items[0] && data.response.items[0].to_id) {
...
function getLike() {
	...
	VK.api('likes.getList',{type: 'post', owner_id: contest.owner, item_id: contest.post, filter: 'likes', friends_only: 0, offset: getRandom(), count: '1'}, workLike);
	}

function workLike(data) {
	if (typeof data.response != "undefined" && data.response.items[0]) {
...
function getUser(id) {
	VK.api('users.get',{fields: 'photo_50', user_ids: id}, workUser);
	}
...
function checkSubscribtion(user) {
	if (typeof user != "undefined") VK.api('groups.isMember',{group_id: contest.owner.replace("-", ""), user_id: user.id, extended: '0'},function(data) { data.user = user; checkUser(data); });
		else nextStep();
	}
